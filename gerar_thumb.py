...
def gerar_thumb(
    ...
    mascara_path: str | Path | None = None,
) -> Image.Image:
    """
    ...
    Args:
        ...
        frame_path:   Caminho direto para uma imagem a usar como frame (opcional).
        mascara_path: Caminho específico para a máscara PNG do projeto (opcional).
    ...
    # Determinar a máscara efetiva a usar
    mascara_efetiva = Path(mascara_path) if mascara_path else MASCARA_PATH
    if not mascara_efetiva.is_absolute():
    ...
    if not mascara_efetiva.exists():
        print(f"  AVISO: Máscara não encontrada: {mascara_efetiva}")
